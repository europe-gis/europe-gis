...
def build_network(network_name, output_layer_count = 1, **kwargs):
    ...
    if network_name == 'linear':
        model = tf.keras.Sequential(
            [
                ...
                tf.keras.layers.Dense(output_layer_count, use_bias=True, activation='sigmoid')
            ]
        )
        return model
